...
public class AGparser {
    ...
    public static void main(String[] args) {
        if(args.length !=2) {
            ...
            System.err.println("java -jar pvm-wsdl.jar REQUIREMENT.xml  SERVICE.wsdl");
            System.exit(1);
        }

        WSSecSpec spec = readSpec(args[0]);
        System.out.println("Specification:    "+spec);
        WSSecSpec wsdl = readWSDL(args[1]);
        System.out.println("Actual WS Policy: "+wsdl);

        PVMCoreResult result = checkWsdlSpec(spec,wsdl);
        System.out.println("Verification result: "+result.getVerificationResult()
                           +" ("+result.getVerificationExplaination()+")");
        System.exit(result.getVerificationResult());
    }
